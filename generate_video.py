...
video_path = output_dir + "/" + video_id + ".mov"
while os.path.isfile(video_path):
    video_path = video_path[:-4] + "-.mov"

import moviepy.editor # pip install moviepy
import ffmpeg as fp
print ('Generating PNG ...')
stream = fp.input(os.path.join(frame_dir, 'frame-'+'%05d.png'))
stream = fp.output(stream, video_path, r=30, vcodec='prores_ks', pix_fmt='yuva444p10', vprofile=4, bits_per_mb=8000, vf='scale=in_color_matrix=bt709:out_color_matrix=bt709')
fp.run(stream)
print("video ready:\n%s" % video_path)
